...
<!-- 通过和 imooc 的沟通，由于 wiki.md 中的讨论，我的理论为纲的思路被打破了，大纲重新调整，改为先局部后整体的“动手”为纲的策略
就按照一个 best practice 下如何制作一个网站的思路，来一步步的 build ，过程中稍微串一下知识点就行了，避免跟那位老师的重复。

demo: 重新设计 http://www.imooc.com/learn/333

第一步；需求分析
 - 没什么好分析的，就是 http://www.imooc.com/view/333 这些要素，弄好看了就行

第二步：框架选择
 - 对比一下 bootstrap 和 initializer 还是用 initializer 手写吧
 - 不用个框架是不行了，引用 github pages/未来逐步引入 jekyll 功能

第三步：出移动版的设计
  - 就照着这个来：http://qd.haoduoshipin.com/
  - 汉堡按钮导航栏还是要有的
    - 如果太麻烦也可以没有，毕竟实现这个不是响应式的核心内容，可以告诉大家这个可以用 bootstrap 或者 web component 来复用人家的
    - http://toy.haoduoshipin.com/gm/material-design/introduction.html 这个挺简单
  - 先选定最窄屏幕 iphone5 ，单位就先用 px 来做
  - 为何要移动优先
  - 引入 material Design
  - 使用 svg 图标系统来完成 qq/weibo/weixin 分析按钮
  - 使用 chrome 移动调试工具
